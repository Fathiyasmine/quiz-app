...
export const QuizProvider = ({ children }) => {
  ...
  // ⏱️ LE TIMER - Cœur du Système

  useEffect(() => {
    // 1️⃣ Vérifier si le timer doit tourner
    // Si le quiz n'est pas démarré OU s'il ne reste plus de temps → on arrête
    if (!quizInProgress || timeRemaining <= 0) return;

    // 2️⃣ Créer une "alarme" qui sonne toutes les secondes
    const timer = setInterval(() => {
      // 3️⃣ Diminuer le temps de 1 seconde
      setTimeRemaining((prev) => {
        // 4️⃣ Si le temps est écoulé (≤ 1 seconde)
        if (prev <= 1) {
          setQuizInProgress(false); // Arrêter le quiz
          return 0; // Mettre le temps à 0
        }

        // 5️⃣ Sinon, enlever 1 seconde
        return prev - 1;
      });
    }, 1000); // 1000 millisecondes = 1 seconde

    // 6️⃣ IMPORTANT : Nettoyer l'alarme quand le composant disparaît
  ...
  // FONCTIONS - Actions qu'on peut faire
  ...
  // ⏱️ Fonction pour AFFICHER le temps au format MM:SS
  ...
  return (
    <QuizContext.Provider
      value={{
        ...
        currentQuestion,
        selectedAnswers,
        timeRemaining, // ⏱️ Le temps restant en secondes
        quizInProgress,
        ...
        previousQuestion,
        setCurrentQuestion,
        formatTime, // ⏱️ La fonction pour afficher MM:SS
      }}
    >
