        ...
        {tabs.map((tab) => (
          <button
            ...
            className={`px-4 py-2 rounded-full border-none cursor-pointer font-semibold text-sm transition-all
              ${
                active === tab
                  ? "bg-linear-to-r from-blue-400 to-blue-300 text-white shadow-md shadow-blue-200"
      ...
      <div className="flex flex-col gap-3">
        {quizItems.map((item, i) => (
          ...
          >
            {/* icone */}
            <div
              className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-md shrink-0 ${item.bg}`}
            >
              {item.icon}
            </div>
